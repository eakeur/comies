...
func (w jobs) Channel(ctx context.Context) (chan OrderNotification, error) {
	ch, ok := w.channel[0]
	if ok {
		return ch, nil
	}
...
func (w jobs) createChannel(ctx context.Context) (chan OrderNotification, error) {
...
func (w jobs) sendToChannel(not OrderNotification) {
	ch, ok := w.channel[0]
	if !ok {
		return
	}
