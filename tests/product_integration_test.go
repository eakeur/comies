...
func TestProductIntegration(t *testing.T) {
	...
	t.Run("should create all products successfully", func(t *testing.T) {
		for i, prod := range products {
			wg.Add(1)
			go func(prod menu.GetProductByKeyResponse, i int) {
				defer wg.Done()
				var (
	...
	t.Run("should fetch products by id successfully", func(t *testing.T) {
		for _, prod := range products {
			wg.Add(1)
			go func(prod menu.GetProductByKeyResponse) {
				defer wg.Done()
				var (
	...
	t.Run("should list products with filters successfully", func(t *testing.T) {
		...
		for _, r := range []ListRequest{
			...
			go func(req ListRequest) {
				...
				assert.Equal(t, req.want, data, "the list received was not the same as expected")
			}(r)
		}
		wg.Wait()
	})
